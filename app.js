...
function toggleAuto(){const on=document.getElementById('auto-toggle').checked;const st=document.getElementById('auto-status');if(location.protocol=='file:'){st.textContent='需服务器模式';return}
if(on){st.textContent='检测中...';fetch('/api/auto/settings',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:'{}'}).then(function(r){return r.json()}).then(function(d){if(d.deps&&d.deps.pyautogui){_doEnableAuto()}else if(!confirm('需要安装 pyautogui + pillow 才能使用桌面自动化。\\n将优先从本地 runtime/auto-deps/ 安装。\\n确认安装？')){document.getElementById('auto-toggle').checked=false;st.textContent='已取消'}else{_doInstallAuto()}}).catch(function(){document.getElementById('auto-toggle').checked=false;st.textContent='检测失败'})
}else{if(!confirm('确认卸载 pyautogui + pillow？\\nUSB-AI 仍可正常使用，仅桌面自动化功能不可用。')){document.getElementById('auto-toggle').checked=true;return}
_doUninstallAuto()}}
function _doEnableAuto(){const st=document.getElementById('auto-status');st.textContent='开启中...';fetch('/api/auto/settings',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:JSON.stringify({enabled:true})}).then(function(r){return r.json()}).then(function(d){var s=document.getElementById('auto-status');s.textContent=d.ok?'✅ 已开启':'❌ '+d.error}).catch(function(){document.getElementById('auto-status').textContent='请求失败'})}
function _doInstallAuto(){const st=document.getElementById('auto-status');st.textContent='安装中...';fetch('/api/auto/install',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:JSON.stringify({action:'install'})}).then(function(r){return r.json()}).then(function(d){if(d.ok){st.textContent='安装成功，开启中...';_doEnableAuto()}else{st.textContent='安装失败: '+d.error;document.getElementById('auto-toggle').checked=false}}).catch(function(){st.textContent='请求失败';document.getElementById('auto-toggle').checked=false})}
function _doUninstallAuto(){const st=document.getElementById('auto-status');st.textContent='卸载中...';fetch('/api/auto/install',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:JSON.stringify({action:'uninstall'})}).then(function(r){return r.json()}).then(function(d){if(d.ok){st.textContent='已卸载 (已关闭)';fetch('/api/auto/settings',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:JSON.stringify({enabled:false})})}else{st.textContent='卸载失败: '+d.error;document.getElementById('auto-toggle').checked=true}}).catch(function(){st.textContent='请求失败';document.getElementById('auto-toggle').checked=true})}
...
function loadCfg(){try{const c=JSON.parse(localStorage.getItem('uai_cfg')||'{}');if(c.apiKey)document.getElementById('apikey').value=c.apiKey;if(c.model)document.getElementById('model').value=c.model;if(c.think){think=true;document.getElementById('c-think').classList.add('on')}if(c.search){search=true;document.getElementById('c-search').classList.add('on')}if(c.auto&&location.protocol!=='file:'){document.getElementById('auto-toggle').checked=true;document.getElementById('auto-status').textContent='查询中...';fetch('/api/auto/settings',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:'{}'}).then(function(r){return r.json()}).then(function(d){var s=document.getElementById('auto-status');if(d.ok&&d.enabled)s.textContent='✅ 已开启';else s.textContent='已关闭'}).catch(function(){})}}catch(e){}}
function saveCfg(){localStorage.setItem('uai_cfg',JSON.stringify({apiKey:document.getElementById('apikey').value,model:document.getElementById('model').value,think,search,auto:document.getElementById('auto-toggle').checked}))}
