...
function toggleAuto(){const on=document.getElementById('auto-toggle').checked;const st=document.getElementById('auto-status');if(location.protocol=='file:'){st.textContent='需服务器模式';return}st.textContent=on?'开启中...':'关闭中...';fetch('/api/auto/settings',{method:'POST',headers:{'Content-Type':'application/json','X-Local-Token':LOCAL_TOKEN},body:JSON.stringify({enabled:on})}).then(function(r){return r.json()}).then(function(d){if(d.ok){st.textContent=on?'✅ 已开启':'已关闭';if(!d.deps.pyautogui&&on)st.textContent='⚠️ 需: pip install pyautogui pillow'}else{st.textContent='❌ '+d.error;document.getElementById('auto-toggle').checked=!on}}).catch(function(){st.textContent='❌ 请求失败';document.getElementById('auto-toggle').checked=!on})}
